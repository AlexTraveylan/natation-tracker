...
async function getAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error('CLIENT_ID, SECRET_CLIENT et REFRESH_TOKEN sont requis dans .env');
  }
...
function extractBestTimesPerDistance(laps: StravaLap[]): Partial<Record<Distance, number>> {
...
export class StravaActivityAdapter implements ActivityProviderPort {
  async getSwimResults(): Promise<SwimResults> {
    ...
    const swims = activities.filter((a) => a.type === 'Swim' || a.sport_type === 'Swim');

    const results: SwimResults = [];
    for (const activity of swims) {
      const lapsRes = await fetch(`https://www.strava.com/api/v3/activities/${activity.id}/laps`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
